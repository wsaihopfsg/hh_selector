...
* IF a specific distance is requested with symbology details, you must evaluate it with 200mm allowance against the tables in Section 2 since this is a handheld reader whose range can be manually moved (e.g., requesting 70mm for a scanner with a 90mm-112mm range is acceptable since the user just have to move his hand by 20mm). In the explanation text however, this manual movement to achieve read range must be high-lighted
